...
while(conv_T ~= 1)
    ...
    [Tedge] = ScalarEquation.ExplicitSolver(Geometry, dtEE, alphac, jin, ...
...
dtp = 0.04; tspan = [0 dtp]; %limits of integration
...
for timesteps = 1:numberoftimesteps
    ...
    for i = 1:Ndrop
        ...
        Ycph = InterpolateQuantities.toDropletLocation(Yedge,jnode,knode,inode,nextknode,drdist,dthetadist,dxdist,dr,dtheta,dx);
        ...
        EvapBCs = [Dd(i) Td(i)];          %Diameter and Temperature BCs;
...
%         %Surface vapor pressure (coupled to temperature):
%         estar = esa*exp(Lvd*MWw/Ru*(1/Tcph - 1/Td(i)) + 2*MWw*sigmad/(Ru*rhod*Dd(i)/2*Td(i)));
%         %Surface specific humidity (coupled to temperature):
%         Ystar = MWw*estar/(Ru*Td(i)*rhoc);
        
        %Solve the system of ODEs for diameter and temperature variation:
        %scalar(1) = Dd; scalar(2) = Td;
        ...
        [~, Scalar] = ode45(fEvap, tspan, EvapBCs);
        Dd(i) = Scalar(end,1); Td(i) = Scalar(end,2);
        
        %% Temperature transport equation (no source yet)
        Tedgenm1 = Tedgen; Tedgen = Tedge;
        [Tedge] = ScalarEquation.ExplicitSolver(Geometry, dtp, alphac, jin, ...
            Twall, Tin, Tedgen, Tedgenm1, N_xtot, N_ytot, N_ztot, ...
            redge, thetaedge, xedge, vedge, wedge, uedge, rnode, thetanode, xnode);
        
        %% Species (water vapor) transport equation (no source yet)
        Yedgenm1 = Yedgen; Yedgen = Yedge;
        [Yedge] = ScalarEquation.ExplicitSolver(Geometry, dtp, Dv, jin, ...
